...
export default async function creditsRoutes(fastify: FastifyInstance) {
  ...
  // ============================================
  // WORKFLOW CREDIT ENDPOINTS
  // ============================================

  /**
   * GET /api/v1/credits/workflow/config - Get workflow credit configuration for user's tier
   */
  fastify.get('/workflow/config', {
    preHandler: [fastify.authenticate],
    schema: {
      tags: ['Credits', 'Workflows'],
      summary: 'Get workflow credit configuration',
    ...
    try {
      ...
      const creditsRemaining = result.rows[0].credits_remaining || 0;
      const workflowConfig = getWorkflowConfigForTier(tier);

      // Check if workflow credits feature is enabled
      const featureFlagResult = await fastify.pg.query<{ enabled: boolean }>(
        `SELECT enabled FROM feature_flags WHERE name = 'workflow_credits_enabled'`
      );
      const workflowCreditsEnabled = featureFlagResult.rows[0]?.enabled ?? false;
      ...
      return {
        ...
        data: {
          ...
          workflowsEnabled: workflowConfig.workflowsEnabled,
          creditMultiplier: workflowConfig.workflowCreditMultiplier,
          freeExecutionsPerDay: workflowConfig.freeWorkflowExecutionsPerDay,
          premiumNodes: PREMIUM_NODES,
        },
      };
    } catch (error) {
      fastify.log.error(error);
      return reply.status(500).send({ success: false, error: 'Failed to get workflow config' });
    }
  });

  /**
   * GET /api/v1/credits/workflow/usage - Get workflow credit usage stats
   */
  fastify.get('/workflow/usage', {
    preHandler: [fastify.authenticate],
    schema: {
      tags: ['Credits', 'Workflows'],
      summary: 'Get workflow credit usage',
    ...
    try {
      ...
      // Get user tier for free execution calculation
      const userResult = await fastify.pg.query(
        'SELECT subscription_tier FROM app_users WHERE id = $1',
        [userId]
      );
      ...
      const todayResult = await fastify.pg.query<{ count: string }>(
      ...
      fastify.log.error(error);
      return reply.status(500).send({ success: false, error: 'Failed to get workflow usage' });
    }
  });

  /**
   * POST /api/v1/credits/workflow/estimate - Estimate workflow execution cost
   */
  fastify.post<{
    Body: {
    ...
    schema: {
      tags: ['Credits', 'Workflows'],
      summary: 'Estimate workflow execution cost',
      ...
      body: {
        ...
        required: ['organizationId', 'flowId'],
      },
    },
  }, async (request: any, reply) => {
    try {
  ...
  /**
   * GET /api/v1/credits/workflow/history - Get workflow credit transaction history
   */
  fastify.get<{
    ...
    schema: {
      tags: ['Credits', 'Workflows'],
      summary: 'Get workflow credit history',
    ...
    try {
      ...
      const result = await fastify.pg.query(
        `SELECT id, type, amount, balance_before, balance_after, reference_type, reference_id, reason, metadata, created_at
         FROM credit_transactions 
         WHERE user_id = $1 AND reference_type = 'workflow_execution'
      ...
      return {
        ...
        data: {
          ...
          pagination: {
            ...
            limit,
            offset,
            hasMore: offset + result.rows.length < parseInt(countResult.rows[0].count, 10),
          },
        },
      };
    } catch (error) {
      fastify.log.error(error);
      return reply.status(500).send({ success: false, error: 'Failed to get workflow history' });
    }
  });

  /**
   * GET /api/v1/credits/unified - Get unified credit summary (AI + Workflows)
   */
  fastify.get('/unified', {
    preHandler: [fastify.authenticate],
    schema: {
    ...
    try {
      ...
      // Get AI generation usage today (excluding workflows and ML service requests)
      const aiUsageResult = await fastify.pg.query(
        `SELECT COALESCE(SUM(ABS(amount)), 0) as used
         FROM credit_transactions
         WHERE user_id = $1 AND amount < 0
         AND reference_type NOT IN ('workflow_execution', 'ml_service_request')
         AND created_at >= $2`,
        [userId, todayStart.toISOString()]
      );
      ...
      // Get workflow usage today
      const workflowUsageResult = await fastify.pg.query(
        `SELECT COALESCE(SUM(ABS(amount)), 0) as used, COUNT(*) as count
         FROM credit_transactions
         WHERE user_id = $1 AND amount < 0 AND reference_type = 'workflow_execution' AND created_at >= $2`,
        [userId, todayStart.toISOString()]
      );
      const workflowCreditsUsedToday = parseInt(workflowUsageResult.rows[0]?.used || '0', 10);
      const workflowExecutionsToday = parseInt(workflowUsageResult.rows[0]?.count || '0', 10);

      // Get user's organization for workflow execution count
      const orgResult = await fastify.pg.query(
        `SELECT o.id FROM organizations o 
         JOIN organization_members om ON o.id = om.organization_id 
      ...
      let totalWorkflowExecutionsToday = workflowExecutionsToday;
      if (orgResult.rows.length > 0) {
        const orgId = orgResult.rows[0].id;
        const execResult = await fastify.pg.query<{ count: string }>(
          `SELECT COUNT(*) as count FROM nodered_execution_logs 
           WHERE organization_id = $1 AND started_at >= $2`,
          [orgId, todayStart.toISOString()]
        );
        totalWorkflowExecutionsToday = parseInt(execResult.rows[0]?.count || '0', 10);
      }
      ...
      return {
        ...
        data: {
          ...
          // Workflows
          workflows: {
            enabled: workflowConfig.workflowsEnabled,
          ...
          // Combined
          totalCreditsUsedToday: aiCreditsUsedToday + mlCreditsUsedToday + workflowCreditsUsedToday,
        },
      };
    } catch (error) {
      fastify.log.error(error);
