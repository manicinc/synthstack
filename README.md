...
SynthStack is **TypeScript-first** - AI/ML features are ready when you need them, but Python dependencies are completely optional.

### Path A: TypeScript-Only (Recommended)

Perfect for web apps, dashboards, and SaaS products that use cloud LLM APIs.

```bash
# No Python required - full-stack TypeScript
...
- Direct LLM integration (OpenAI, Anthropic, OpenRouter) - no Python needed
- Optional: Add `@synthstack/ts-ml-service` (NestJS) for RAG/embeddings in TypeScript

### Path B: Full AI Stack

Add Python services when you need advanced ML capabilities.
...
- Advanced ML pipelines (scikit-learn, PyTorch)
