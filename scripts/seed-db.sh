...
echo "🌱 Seeding SynthStack database..."
echo "Mode: $MODE"

case $MODE in
  local)
    # Direct psql connection (requires psql installed)
    psql -U postgres -d synthstack -f "$SEED_FILE"
    ;;
  docker)
    # Via docker container
    docker exec -i synthstack-postgres psql -U postgres -d synthstack < "$SEED_FILE"
    ;;
  compose)
    # Via docker compose
    docker compose exec -T postgres psql -U postgres -d synthstack < "$SEED_FILE"
    ;;
  remote)
